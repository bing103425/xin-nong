...
export default {
  ...
  methods: {
    editAdd(item){
      wx.redirectTo({
        url: '/pages/addShouHuoDiZhi/main?mobile='+item.mobile+'&&consigner='+item.consigner+'&&province='+item.province+'&&city='+item.city+'&&district='+item.district+'&&address='+item.address+'&&alias='+item.alias+'&&id='+item.id
      })
    },
    deletAdd(index,id){
      var that = this
